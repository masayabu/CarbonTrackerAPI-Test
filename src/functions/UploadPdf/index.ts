...
async function UploadPdf(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  try {
    // CORSヘッダーを設定
    const headers = {
    ...
    // リクエストボディを取得
    const body = await request.json() as UploadPdfRequest;
    ...
    context.log(`PDFファイルアップロード開始: ${uniqueFileName}, サイズ: ${buffer.length} bytes`);
    ...
    context.log(`PDFがアップロードされました: ${pdfUrl}`);
    ...
    context.error('PDFアップロードエラー:', error);
    return {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
